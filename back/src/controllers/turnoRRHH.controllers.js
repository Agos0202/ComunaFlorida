...
export const getTurno = async (req, res) => {
    try {
        const [rows] = await pool.query("SELECT * FROM Turnorrhh");
        res.json(rows);
    } catch (error) {
        console.error(error);
...
export const getTurn = async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await pool.query("SELECT * FROM Turnorrhh WHERE id = ?", [id]);
        if (rows.length === 0) {
...
export const postTurno = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(
            "INSERT INTO Turnorrhh (nombre, apellido, documento, fecha, horario, telefono, correo) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
export const putTurno = async (req, res) => {
    try {
        ...
        const { nombre, apellido, documento, fecha, horario, telefono, correo } = req.body;
        const [result] = await pool.query(
            "UPDATE Turnorrhh SET nombre = ?, apellido = ?, documento = ?, fecha = ?, horario = ?, telefono = ?, correo = ? WHERE id = ?",
...
export const deleteTurno = async (req, res) => {
    try {
        const { id } = req.params;
        const [result] = await pool.query("DELETE FROM Turnorrhh WHERE id = ?", [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: "Turno no encontrado" });
        }
