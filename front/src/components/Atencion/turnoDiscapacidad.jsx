...
function Turno() {
  ...
  const obtenerTurno = () => {
    axios.get("http://localhost:2500/api/TurnoDiscapacidad")
      .then(response => {
  ...
  const generarPDF = (turnoData) => {
    ...
    doc.setFontSize(20);
    doc.text("TURNO DE ATENCION - AREA DISCAPACIDAD", 20, 40);
  ...
  const agregarTurno = () => {
    ...
    axios.post("http://localhost:2500/api/TurnoDiscapacidad", turnoData)
      .then(() => {
  ...
  const Update = () => {
    axios.put(`http://localhost:2500/api/TurnoDiscapacidad/${id}`, {
      id: id,
      nombre: nombre,
      apellido: apellido,
      documento: documento,
  ...
  const eliminarTurno = (id) => {
    axios.delete(`http://localhost:2500/api/TurnoDiscapacidad/${id}`)
      .then(() => {
