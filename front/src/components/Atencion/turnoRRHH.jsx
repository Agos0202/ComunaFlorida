...
function Turno() {
  ...
  const obtenerTurno = () => {
    axios.get("http://localhost:2500/api/TurnoRRHH")
      .then(response => {
  ...
  const generarPDF = (turnoData) => {
    ...
    doc.setFontSize(20);
    doc.text("TURNO DE ATENCION - AREA RRHH", 20, 40);
  ...
  const agregarTurno = () => {
    ...
    axios.post("http://localhost:2500/api/TurnoRRHH", turnoData)
      .then(() => {
  ...
  const Update = () => {
    axios.put(`http://localhost:2500/api/TurnoRRHH/${id}`, {
      id: id,
      nombre: nombre,
      apellido: apellido,
      documento: documento,
  ...
  const eliminarTurno = (id) => {
    axios.delete(`http://localhost:2500/api/TurnoRRHH/${id}`)
      .then(() => {
