...
function AlumnoBasquet() {
  ...
  const obtenerAlumnos = () => {
    axios.get("http://localhost:2500/api/AlumnoBasquet")
      .then(response => {
  ...
  const Update = () => {
    axios.put(`http://localhost:2500/api/AlumnoBasquet/${id}`, {
        id: id,
        nombre: nombre,
        apellido: apellido,
        documento: documento,
  ...
  const eliminarAlumno = (id) => {
    axios.delete(`http://localhost:2500/api/AlumnoBasquet/${id}`)
      .then(() => {
  ...
  const generarPDF = () => {
    ...
    pdf.text("Listado de Alumnos - Basquet", 40, 40);
    ...
    // Save the PDF
    pdf.save("listado_alumnos_Basquet.pdf");
  };
  ...
  return (
    <div>
      ...
      <h1 className="h1">BASQUET 2024</h1>
...
<div>
      <button onClick={generarPDF}>Imprimir Listado</button>
  </div>
